...
// === MAX7219 add-on (shows Now Serving on 7-seg) ===
...
void setup() {
  pinMode(PIN_SERVE, INPUT_PULLUP);
  pinMode(PIN_BUZZ, OUTPUT);
...
void loop() {
  ...
  bool now = readButton(PIN_SERVE);
  if (now && !prev) {
    delay(50);
    if (readButton(PIN_SERVE)) {
      if (serving < ticket) {
        serving++;
        if (serving > 999) serving = 1;
        ...
        Serial.print('S');
        char buf[4];
        snprintf(buf, sizeof(buf), "%03u", (unsigned)serving);
        Serial.println(buf);
      } else {
        // no one waiting
        beep(40, 900);
  ...
  // Receive peer updates (ticket from dispenser)
  while (Serial.available()) {
    ...
    if (type == 'T') {
      ...
      char d1 = Serial.read();
      char d2 = Serial.read();
      ticket = (d0-'0')*100 + (d1-'0')*10 + (d2-'0');
      while (Serial.available()) {
        if (Serial.read()=='\n') break;
      }
      eepromWrite16(EE_TICKET, ticket);
      drawLCD();
      // 7-seg stays showing serving on counter
    }
  }
}